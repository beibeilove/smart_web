        ...
        <a-row :gutter="48">
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="所属区域">
              <a-select v-model="queryParam.region" placeholder="请选择区域">
...
<script>
  ...
  import { getNoRecognizedList, getAreaList } from '@/api/base'
    ...
    data () {
      return {
        ...
        loadData: parameter => {
          ...
          return getNoRecognizedList(Object.assign({}, this.queryParam, this.listQuery))
            .then(res => {
    ...
    methods: {
      ...
      handleExport() {
        ...
        require.ensure([], () => {
          ...
          const tHeader = ['序列号', '问题描述', '提问时间']
          const filterVal = ['id', 'question', 'created']
          const list = this.loadDataArr
          const data = this.formatJson(filterVal, list)
          export_json_to_excel(tHeader, data, '未识别问题统计'+this.queryParam.startdate + '~' + this.queryParam.enddate)
